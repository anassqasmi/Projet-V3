			...
			<header><h3>Nouvelle tache</h3></header>
				...
				<div class="module_content">
						...
						<?php //getTache($_GET['id']);?>
							<label>Date : </label>
							<input type="date" size="50%" id="selectedDate" onselect="datee()" value="<?php echo $_SESSION['td'];?>"/>
						</fieldset>
						<fieldset>
							<label>Tache : </label>
							...
							<select style="width:92%;" id=selectCat >
								<?php
									...
									foreach ( $catArry as $catego ) {
										if($_SESSION['tca'] . " " == $catego . " ")
										{ 
											echo '<option selected="selected">' . $catego . '</option>';
										}else 
										{
							...
							<select style="width:92%;" id="selectTag">
								<?php
									...
									foreach ( $tagArry as $tagArry ) {
										if($_SESSION['tt'] . " " ==  $tagArry . " ")
										{
											echo '<option selected="selected">' . $tagArry . '</option>';
										}else {
				...
				<div class="submit_link">
					<select id="selectQui">
						...
						<?php
							...
							foreach ( $tagArry as $tagArry ) {
								if($tagArry[0] . " " == $_SESSION['tci'] . " ")
								{
									echo '<option selected="selected" value ='. $tagArry[0] . '>' . $tagArry[1] . '</option>';
								}else {
									echo '<option value ='. $tagArry[0] . '>' . $tagArry[1] . '</option>';
								}		
							}?>
							<option class="select-dash" disabled="disabled">Groupes  ---------</option>
						<?php 
							readGroups();
